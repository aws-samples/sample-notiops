...
def capability_answer(locale: str = "zh") -> str:
    ...
    return _pick(
        ...
        # 「安全」2026-09-11 起不再是独立主题（聊天并入「故障调查」，看板变成入口），
        # 所以这里也不能再把它列成一条并列的主题 —— 否则客户按这句话去左侧找「安全」会找不到。
        "- **故障调查与安全** — 看指标与日志、翻审计事件、只读巡检 EC2 / RDS 等资源；"
        "也包括只读的安全态势检查（安全组规则、对外暴露面、Security Hub / GuardDuty 发现等）；"
        ...
        "用法：左侧切换主题（安全态势等看板在「调查」输入框上方，或侧栏「安全」直达），"
        "输入框上方可选目标 AWS 账号与模型。"
        ...
        "- **Investigation & security** — read metrics and logs, search audit events, inspect "
        "resources like EC2 / RDS read-only; read-only security posture checks are part of this "
        "too (security group rules, public exposure, Security Hub / GuardDuty findings…); turn on "
        "**Deep Dive** to hand the problem to AWS DevOps Agent for root-cause analysis.\n"
        ...
        "How to use it: switch topics on the left (security posture and the other dashboards sit "
        "above the Investigation input box, or straight from \"Security\" in the sidebar); pick the "
        "target AWS account and model above the "
