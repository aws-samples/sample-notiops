"""
LLM 模型目录读取（webchat agent 侧）—— DDB 单一真源 + TTL 缓存 + 内置兜底。
...
⚠️ 本文件在 **IM 侧有一份对等实现**（`core/llm_config.py`）。两处是不同部署单元
（AgentCore zip vs ECS 镜像），无法共享库，改动务必同步——差异只应体现在
`_SURFACE` 常量与 per-surface 输出目标上。

热生效（spec R4）：
  · TTL 60s 兜底轮询；
  · BFF 在每条消息的 payload 里带**服务端生成**的 `generation`，本模块发现与本地
    缓存不同即绕过 TTL 强制刷新（强一致读）。**"下一条消息即生效"受限速约束**：
    强刷间隔内的第二次保存要等 TTL —— 所以该间隔必须严格小于 TTL，否则强刷窗口
    被完全覆盖、对最坏情况零改善（见 `_FORCED_REFRESH_MIN_INTERVAL`）。
  · 与 IM 侧的差别在此：IM 链路上没有 BFF，也没有任何调用方传 generation，
    因此 IM 只有 TTL（最坏 60s）。别把这段行为套到 IM 上。
...
# 本模块服务的端。IM 侧对等实现只改这一个常量（+ 输出目标）。
_SURFACE = "webchat"

# 输出上限目标：足够覆盖本产品最长的分析/报告类回答（含 reasoning）。
# 实际值 = min(模型 hard_output_limit, 本目标)，可被目录的 output_override[surface] 覆盖。
_OUTPUT_TARGET = 32768
