 ...
 * @param {object} opts       { title, subtitle, meta:{task_id,execution_id,agent_space_id,created_at,updated_at}, status, priority, footer }
 *
 * `footer` 默认 "Generated by NotiOps Deep Dive"（老行为一字不变）。**必须能改**：
 * 这个模板已经不只服务「深度调查」了 —— 阿里云 STAROps 的报告也用它渲染，落款写
 * "Deep Dive" 就是把 A 功能的名字盖在 B 功能的产物上，客户会去找一个不存在的入口。
 */
export function renderReport(summaryMd, { title = "NotiOps Report", subtitle = "", meta = {}, status = "", priority = "", footer = "" } = {}) {
...
function generateHtmlReport({ summaryMd, status, priority, detailType, taskId, executionId, agentSpaceId, createdAt, updatedAt, footer = "" }) {
  ...
  const _footer = escAttr(footer || "Generated by NotiOps Deep Dive");

  // 元信息只渲染**有值的**格子。原来是 5 个格子无条件铺满：DevOps 那条路径 5 个都有值，
  // 看不出问题；换成阿里云 STAROps（根本没有 task/execution/agent space 这些概念）就会
  // 出现 5 个带 AWS 标签的**空格子** —— 客户看到 "Execution ID" 空着，第一反应是"这份报告
  // 缺数据/出错了"，而不是"这个字段与本报告无关"。空着不如不出现。
  const metaCells = [
    ["Task ID", _taskId, true],
    ["Execution ID", _executionId, true],
    ["Started", _createdAt, false],
    ["Completed", _updatedAt, false],
    ["Agent Space", _agentSpaceId, true],
  ]
    .filter(([, v]) => v)
    .map(([l, v, mono]) => `<div class="mc"><div class="l">${l}</div><div class="v${mono ? " mono" : ""}">${v}</div></div>`)
    .join("\n    ");
  const metaBlock = metaCells ? `<div class="meta">\n    ${metaCells}\n  </div>` : "";
  // 同理：priority 为空时不出 badge。DevOps 直连一直传 `priority: ""`，于是现网每份报告
  // 头上都挂着一个写着 "Priority:" 后面什么都没有的蓝色胶囊 —— 纯噪音，顺手一起修掉。
  const priorityBadge = _priority
...
<div class="page">
  ...
  <!-- 2026-09-13：去掉 "(Direct)" —— 界面上只剩一个「深度调查」/ Deep Dive（转交那一条已撤掉
       入口），"(Direct)" 失去了对照对象，落在客户手上的报告里只会让人猜"还有个不直连的版本？"。 -->
