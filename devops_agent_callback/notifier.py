...
logger = logging.getLogger("devops_agent_callback.notifier")
...
def send_notifications(content: str, feishu_secret: dict) -> dict:
    ...
    try:
        chat_ids_str = (
            feishu_secret.get("notify_chat_ids", "") if feishu_secret else ""
        )
        if chat_ids_str:
            chat_ids = [c.strip() for c in chat_ids_str.split(",") if c.strip()]
            if chat_ids:
                feishu_sent = _send_feishu_notifications(
                    chat_ids, content, feishu_secret
                )
