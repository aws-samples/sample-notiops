...
The bot calls **AWS Knowledge MCP** to retrieve official docs, then answers with the chat's current conversational model (**Claude Sonnet 5** by default, see §7), **with verifiable sources attached**:
...
- **General chat**: a new conversation when you haven't entered any topic. It opens by asking who should answer — the **conversation object** (NotiOps / your own DevOps Agent); see §12.2.
- **Default model**: a new conversation defaults to **Claude Sonnet 5** (the exact default is set by your admin on the Admin → Models page).
- **Right-side Sources / Investigation panel**: shows tool calls and source pass-through, plus the live "Investigation" panel under the Investigate topic (see §12.4).

Toggles available in the top/toolbar area: **multi-account selector**, **model switching**, **web search toggle** (see §12.8).

### 12.2 General chat: pick who answers (NotiOps / your own DevOps Agent)

A **general chat** (a new conversation when you haven't entered any left-side topic) opens with a two-segment **conversation object** picker that decides **who answers this conversation**:

| Object | Who answers | What it's good at | Cost / prerequisite |
|---|---|---|---|
| **NotiOps** (default) | The NotiOps agent (models served via Amazon Bedrock) | The wider view — inspection, investigation, cases, official-doc retrieval, Skills, web search, cost | Consumes the Bedrock model capacity configured for this deployment |
| **DevOps Agent** | **The AWS DevOps Agent in your own account**; its content is passed through verbatim | On the ground — live diagnostics on a single resource or event | **0 tokens on the NotiOps side** (usage is billed to your own DevOps Agent); **no model setup** — nothing has to be enabled in Bedrock, so a fresh deployment with no models configured can use this side right away |

**Key points:**

- **It's skippable** — ignore it and start typing, and you get NotiOps.
- **Once you send the first message, the object is locked for that conversation**; to switch, start a new conversation. The **NotiOps / DevOps Agent** tag in the conversation header tells you which one is active.
- After you pick DevOps Agent, the composer toolbar **slims down**: the **web search toggle** and the **model selector** disappear (this turn doesn't go through our models, so picking one is meaningless), leaving a single **"Deep investigation"** checkbox —
  - **unchecked** (default): instant Q&A with **streaming output**, the same experience as the DevOps Agent's own console;
  - **checked**: this turn runs a full multi-signal root-cause investigation and produces a report (usually minutes).
- **This path does not attach NotiOps tools or Skills**; any action that needs human approval must be completed in the DevOps Agent console. The starter cards also change to four diagnostic questions (recent anomalies / why an EC2 instance rebooted / whether RDS is healthy / recent changes) instead of pointing you at cost, cases, or Skills — use NotiOps or the matching topic for those.
- **Prerequisite**: the account must be onboarded to AWS DevOps Agent. When it isn't, this segment is **greyed out with the reason spelled out** (no Agent Space in this deployment account / the selected account isn't onboarded), rather than letting you send a turn and then hit an error.

### 12.3 Notifications topic: Health Dashboard + inbox red dot
...
### 12.4 Investigate topic: live process panel + mitigation hand-off + escalate to human
...
**Two ways to dispatch it** (both toggles live in the composer toolbar; only one applies at a time):

- **Deep investigation** — a NotiOps model first reads your question and turns it into an investigation request before dispatching (it fills in what you left implicit, and answers any conceptual question you tucked in).
- **Deep investigation (Direct)** — the same DevOps Agent deep investigation, but it **calls the API directly without an LLM**, so it **costs 0 tokens**. Trade-off: your wording is **passed through as-is** (no smart rewrite), and conceptual questions aren't answered first.

The report, the right-side "Investigation" panel, and the buttons after the conclusion are identical on both paths; the direct path shows no token usage (there is none).
...
### 12.6 Cases topic: two ways to file a case
...
- **Who executes a Skill once you pick one**: if this turn is handed to your own DevOps Agent (deep investigation / deep investigation Direct / a general chat where you picked DevOps Agent), the Skill chip above the composer carries a **DevOps Agent** marker. The two behave differently, and the UI says which:
  - **Deep investigation** (hand-off): the DevOps Agent uses **its own copy** of the Skill, so you **must publish the Skill to DevOps Agent from the "Skills" page first** — otherwise it won't be activated on that turn.
  - **The two direct paths**: the Skill **body is inlined into that turn**, so **no publishing is needed**; if the Skill ships `references/` files, however, those aren't reachable on a direct path.

### 12.8 Multi-account / multi-model / web search / What's New / "/" commands

- **Model switching**: defaults to **Claude Sonnet 5**, with **Claude Opus 5**, **Claude Sonnet 4.6**, **Claude Haiku 4.5**, **Amazon Nova Pro**, **DeepSeek V3.2**, **GLM 5**, **Grok 4.6**, and the **GPT-5.6** family (Terra / Sol / Luna) also available (all accessed via Amazon Bedrock; third-party models run via Bedrock, not direct vendor APIs). It **remembers your model preference per session**, and **each reply is signed with the model used**.
...
- **A general chat's conversation object is locked once you send the first message** and cannot be switched mid-conversation; a conversation answered by DevOps Agent **has no NotiOps tools or Skills attached**, and any action needing approval must be completed in the DevOps Agent console.
- **Every DevOps Agent capability (deep investigation / Direct / the DevOps Agent object in a general chat) requires the account to be onboarded to AWS DevOps Agent**; when it isn't, the matching toggle is greyed out with the reason shown.
