...
| 2 | 系统账户逻辑视图 | IM 机器人（ECS）与各业务路径如何编排汇聚？ | SA / 架构师 / 后端开发 | 34 |
...
**关键标注**：
- IM 机器人(补充入口)是常驻 **ECS Fargate** 进程，直接 `import core/` 处理意图分类 / 对话 / 进度轮询，**这条 IM 路径不经 API Gateway,也不使用 AgentCore Runtime / Memory / Gateway / Agent 容器**;主入口 Web Chat 则相反 —— 跑在 Bedrock **AgentCore Runtime** 上,经 BFF Lambda + Function URL(SSE)对外(见 WebChatStack,`web-chat-stack.ts`)
- 对话工具调用：`core.bedrock_chat` 通过 hosted AWS Knowledge MCP（`knowledge-mcp.global.api.aws`）+ `sidecars/` 的 pricing / cost MCP 检索官方数据
...
**分三段**：
- Part A（步骤 1-20）：IM 对话流程，含飞书流式卡片伪流式、`core.ddb_state` 条件写幂等去重（`event#` 行）、ECS 进程内异步处理
...
**当前架构关键点**（参见图右下方蓝色便签）：
1. IM 机器人(补充入口)是常驻 ECS Fargate 进程，import `core/`，这条 IM 路径不经 API Gateway、不使用 AgentCore Runtime;主入口 Web Chat 反之运行在 Bedrock AgentCore Runtime 上(WebChatStack)
...
| IM 机器人（ECS Fargate） | `infra/lib/bot-stack.ts` + `platforms/{feishu,slack,dingtalk}/app/main.py` |
| IM Bot 去重/幂等 | `core/ddb_state.py`（`event#` 行条件写）+ 各平台 `app/main.py` |
| 机器人派发 DevOps Agent | `core/webhook_dispatch.py` + `shared/devops_agent.py` |
| 对话工具调用（MCP） | `core/aws_docs_mcp.py` + `core/mcp_http_client.py` + `sidecars/` |
