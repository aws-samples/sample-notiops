...
数据层 **全部使用 DynamoDB**(后台 3 张核心表,另加 Web Chat 的 `notiops-web-chat` 单表),**没有任何 RDS / PostgreSQL / SQL**。**IM 机器人** 是常驻 **ECS Fargate** 进程(直接 `import core/`),这条 IM 路径不经 API Gateway,也不使用 AgentCore Runtime / Agent 容器;**Web Chat 主入口则相反** —— 它跑在 Bedrock **AgentCore Runtime** 上,经 BFF Lambda + Function URL(SSE)对外(见下方「部署栈」的 WebChatStack)。
...
│                                                                                              │
│  ── 路径 C：IM 机器人（ECS Fargate 常驻）──                                                  │
│                                                                                              │
│  飞书 ──长连 WS──┐                                                                           │
│  Slack ──Socket──┤──▶ platforms/{feishu,slack,dingtalk} (ECS Fargate task, import core/)    │
│  钉钉 ──Stream───┘        │                                                                  │
│                           ├─ core.bedrock_intent  意图分类(8 类 command)                    │
│                           ├─ core.bedrock_chat    通用对话 + 三层零变更防御 + AWS Docs MCP   │
│                           ├─ core.webhook_dispatch HMAC 签名派发 DevOps Agent generic webhook│
│                           ├─ core.progress_poller daemon 轮询 aidevops:ListJournalRecords    │
...
│  SNS │ SQS DLQ │ Cognito User Pool │ Custom Event Bus │ ECS Cluster + ECR(bot 镜像)         │
└──────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| `BotStack` | IM 机器人(补充入口):VPC + ECS Fargate Cluster + ECR(bot 镜像)+ 每平台一个 Fargate Service(feishu / slack / dingtalk)+ MCP sidecar |
...
| 资源 | 说明 |
|------|------|
| VPC (BotStack) | ECS Fargate task 运行;bot 走公有子网长连接出网 |

> 后台 Lambda 不入 VPC(直接走 AWS API),数据层是 DynamoDB,无需隔离子网托管数据库。
...
| ECS Fargate — IM bots (`platforms/{feishu,slack,dingtalk}`) | 每平台一个常驻 Service;feishu/slack 当前 GA,dingtalk 为 Phase 2（适配代码完整保留，v1 `setup.sh` 默认不开放）;`desiredCount` 条件设置(未选中的平台为 0) |
...
| 资源 | 说明 |
|------|------|
| Custom Event Bus | `notiops-devops-events`,接收业务账户跨账户转发的 `aws.aidevops` 事件(白名单 + source 条件双重校验) |
...
> 每个业务账户独立部署一个 AgentSpace(`AccountType=monitor`),不采用官方系统账户单一 AgentSpace 模式。这是刻意的架构决策:让每个业务账户各自持有 Agent 服务角色与信任边界,跨账户只经白名单校验的事件回调聚合,避免单一共享 AgentSpace 成为跨账户权限的集中风险点。IM bot 与调查回调这条路径不依赖 AgentCore Runtime / Memory / Gateway(Web Chat 主入口则运行在 AgentCore Runtime 上)。
...
| ECS Fargate + ECR | IM 机器人常驻进程(feishu / slack / dingtalk)+ MCP sidecar 镜像 |
...
| 组件 | 技术 |
|------|------|
| 后端 | Python (Lambda + ECS) + boto3 |
| 数据层 | Amazon DynamoDB(3 表,单表设计 + GSI) |
| IM 机器人 | ECS Fargate 常驻进程(飞书 lark-oapi / Slack slack_bolt / 钉钉 Stream Mode) |
| 前端 | React + TypeScript + Cloudscape |
| IaC | AWS CDK (TypeScript),3 个 Stack(WebChatStack + NotiOpsBackendStack + BotStack) |
| 认证 | Amazon Cognito |
...
├── platforms/                    # IM 平台适配 + ECS 容器（feishu / slack / dingtalk）
├── sidecars/                     # ECS sidecar 镜像（awslabs pricing / cost MCP server 包装）
