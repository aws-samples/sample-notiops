...
/** Cases dashboard 右侧停靠面板（复用 .src-panel 停靠/拖宽/关闭机制,同 FinopsDashboardPanel）。 */
export default function CasesDashboardPanel({
    ...
    <>
      ...
      <aside className={"src-panel" + (open ? " open" : "")} style={open ? { width } : undefined}>
        ...
        <div className="src-body" style={{ padding: 0 }}>
          {open && dashboardId && <CasesDashboard dashboardId={dashboardId} data={data} can={can} />}
        </div>
      </aside>
    </>
  );
