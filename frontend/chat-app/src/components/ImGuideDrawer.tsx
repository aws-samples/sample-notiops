/**
 * 「配置 IM 机器人」右侧抽屉的**通用外壳**（飞书 / 钉钉共用）。
 *
 * 为什么是抽屉而不是新页/新弹窗:客户是**照着步骤填上一页的输入框**，步骤和输入框
 * 必须同屏可见 —— 弹窗会盖住表单，跳页会丢掉已填一半的凭证（本页表单没有草稿保存）。
 * 所以:桌面端右侧停靠、不遮挡左边的表单；窄屏退化为覆盖式并给遮罩（CSS 里做）。
 *
 * 内容按平台分文件（content/feishuGuide.ts、content/dingtalkGuide.ts，按 locale 整块给），
 * 本文件只管渲染与开合。Esc 关闭 —— 抽屉打开时它是最上层，键盘用户不该被困在里面。
 *
 * 2026-09-08 加钉钉时从 FeishuGuideDrawer 提取出来:两个平台的抽屉**只差内容与三条
 * 文案 key**，复制一份 130 行的渲染逻辑的真实代价是「改了飞书那份忘了钉钉那份」，
 * 而抽屉出问题的方式全是静默的（步骤看不见 / 复制按钮不动），不会有人报错。
 * `FeishuGuideDrawer` / `DingTalkGuideDrawer` 现在都是十几行的壳。
 *
...
import type { GuideBlock } from "../content/feishuGuide";
 ...
 *
 * 为什么值得单独做一块:配 IM 这条路上，除了这串 URL，其余每一步都在 IM 的网页和本页之间
 ...
 */
function WebhookUrlBox({ url, labelKey, missingKey }: { url: string; labelKey: string; missingKey: string }) {
  const t = useT();
  const [copied, setCopied] = useState(false);

  if (!url) return <div className="imd-warn">{t(missingKey)}</div>;
  ...
  return (
    <div className="imd-urlbox">
      <input id="imd-webhook-url" className="imd-url" readOnly value={url}
        onFocus={(e) => e.currentTarget.select()} aria-label={t(labelKey)} />
    ...
    <>
      ...
      <aside className={"imd-panel" + (open ? " open" : "")} aria-hidden={!open}>
        <div className="imd-head">
          <div>
            <div className="imd-title">{t(titleKey)}</div>
            <div className="imd-sub">{t(subKey)}</div>
          </div>
        ...
        <div className="imd-body">
          {blocks.map((b, i) => (
            <Block b={b} webhookUrl={webhookUrl} urlLabelKey={urlLabelKey} urlMissingKey={urlMissingKey} key={i} />
          ))}
        </div>
      </aside>
    </>
