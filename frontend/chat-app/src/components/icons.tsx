...
// GitHub 官方 Octicon「mark-github」（GitHub 商标）—— 左下角「给我们加星」那个入口用它。
//
// 来源：primer/octicons `build/svg/mark-github-16.svg`，@primer/octicons@19.36.0，MIT（代码）。
//   取回日期 2026-09-11；d 属性 sha256 = b9b3b853d453448426fabeb3bde9df1660b7daa0e458b0eadbef0dcb86226deb
//   （721 字符，与 raw.githubusercontent 上的 primer/octicons 逐字节一致）。
//
// ⚠️ 这一个**故意不走 `base(size)`**，也不是描边图标，别"统一"它：
//   · viewBox 是 16×16（不是其余图标的 24×24），套 24 的 viewBox 会把 mark 缩成一小团；
//   · 官方文件里 path **没有** fill / stroke 属性 → 它是一个实心填充形状（nonzero，单条子路径）。
//     加 stroke / 改成 fill="none" 会画出一个空心怪图形；
//   · d 一律**原样照抄、不许重绘或化简** —— octicons 的**代码**是 MIT，但 GitHub 的
//     **商标**限制仍然在：不许改形、拉伸、加渐变或阴影，只允许单色（白/黑）。
//     我们这里用 currentColor 跟着侧栏文字色走，属于单色，合规；用途是"链到 GitHub 仓库"，
//     属于 GitHub 明确许可的用法。
//   台账见 docs/ATTRIBUTION.md。
export const IconGitHub = ({ size = 16 }: P) => (
...
// 多云 — 云朵（Admin「多云」页：登记另一朵云的只读凭据）
export const IconCloud = ({ size = 16 }: P) => (
  <svg {...base(size)}><path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z" /></svg>
);
