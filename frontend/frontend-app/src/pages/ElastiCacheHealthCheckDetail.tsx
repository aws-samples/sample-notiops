...
import { getElastiCacheHealthCheckDetail } from "../api";
...
export default function ElastiCacheHealthCheckDetail() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    getElastiCacheHealthCheckDetail(id)
    ...
    return (
      <Container>
        <Box textAlign="center" padding="xxl" color="text-status-error">
          {error || "报告不存在"}
          <Box margin={{ top: "m" }}>
            <Button onClick={() => navigate("/elasticache-health-check")}>
              返回报告列表
            </Button>
          </Box>
        </Box>
  ...
  return (
    <SpaceBetween size="l">
      <Header
        ...
        actions={
          <Button
            onClick={() => navigate("/elasticache-health-check")}
            iconName="arrow-left"
          >
            返回报告列表
          </Button>
