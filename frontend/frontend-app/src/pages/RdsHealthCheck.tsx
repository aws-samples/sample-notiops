/**
 * RDS AI 智能巡检报告页面。
 * 展示 AI 驱动的 RDS 数据库健康巡检报告列表，支持一键生成和轮询。
 */
...
import { getRdsHealthCheckList, triggerRdsHealthCheck, deleteRdsHealthCheckBatch, triggerPipeline, getPipelineStatus } from "../api";
...
export default function RdsHealthCheck() {
  ...
  // 进度状态: idle | collecting | generating | done | error
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const resp = await getRdsHealthCheckList(params);
      ...
      console.error("Failed to load RDS health check reports", errMsg(e));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // 检测 generating 状态，自动恢复轮询（页面刷新后恢复）
  useEffect(() => {
  ...
  const handleDeleteSelected = async () => {
    ...
    setDeleting(true);
    try {
      await deleteRdsHealthCheckBatch(selectedItems.map((i) => ({ date: i.report_date, type: i.report_type, account: i.account_id ?? undefined })));
      setSelectedItems([]);
      await fetchData();
    } catch (e) {
  ...
  return (
    <SpaceBetween size="l">
      <Header
        ...
        description="AI 驱动的 RDS 数据库健康巡检报告"
      >
        RDS AI 智能巡检
      </Header>
      ...
      {loading ? (
        ...
        <Table
          ...
          onRowClick={({ detail }) =>
            navigate(`/rds-health-check/${detail.item.id ?? detail.item.report_date}`)
          }
          empty={
            <Box textAlign="center" padding="xxl">
              暂无 AI 巡检报告
