/**
 * ElastiCache 闲置资源报告页面。
 * 展示 ElastiCache 特有的闲置指标列。
 */
...
export default function WasteElastiCache() {
  const navigate = useNavigate();
  const [allItems, setAllItems] = useState<ECWasteItem[]>([]);
  ...
  const [selectedItems, setSelectedItems] = useState<ECWasteItem[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await getWasteReportList({ page: "1", page_size: "10000", resource_type: "elasticache" });
      const items = (resp.data.items ?? resp.data ?? []) as ECWasteItem[];
      setAllItems(items);
    } catch (e) {
      console.error("Failed to load ElastiCache waste report", errMsg(e));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = async () => {
    try {
      const resp = await exportWasteReportCSV({ page: "1", page_size: "10000", resource_type: "elasticache" });
      const blob = new Blob([resp.data], { type: "text/csv" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = "elasticache_waste_report.csv"; a.click();
  ...
  const cacheHitRate = (item: ECWasteItem) => {
    if (item.cache_hits == null || item.cache_misses == null) return "-";
    const total = item.cache_hits + item.cache_misses;
    if (total === 0) return "0%";
    return ((item.cache_hits / total) * 100).toFixed(1) + "%";
  };

  const columns = [
    { id: "instance_id", header: "实例 ID", cell: (item: ECWasteItem) => item.instance_id, sortingField: "instance_id", width: 240 },
    { id: "account_id", header: "账户", cell: (item: ECWasteItem) => item.account_id, width: 130 },
    { id: "region", header: "区域", cell: (item: ECWasteItem) => item.region, width: 140 },
    { id: "instance_class", header: "规格", cell: (item: ECWasteItem) => item.instance_class, width: 150 },
    { id: "engine", header: "引擎", cell: (item: ECWasteItem) => item.engine, width: 100 },
    {
      id: "idle_score", header: "闲置评分", width: 100,
      cell: (item: ECWasteItem) => <Badge color="red">{item.idle_score?.toFixed(1)}</Badge>,
    },
    {
      id: "value_score", header: "价值评分", width: 100,
      cell: (item: ECWasteItem) => item.value_score?.toFixed(1) ?? "-",
    },
    {
      id: "consecutive_low_days", header: "连续低阈值天数", width: 130,
      cell: (item: ECWasteItem) => item.consecutive_low_days,
    },
    {
      id: "cpu_utilization", header: "Engine CPU 利用率 (%)", width: 150,
      cell: (item: ECWasteItem) => item.cpu_utilization != null ? item.cpu_utilization.toFixed(2) : "-",
    },
    {
      id: "connections", header: "连接数", width: 90,
      cell: (item: ECWasteItem) => item.connections ?? "-",
    },
    {
      id: "free_storage_or_memory", header: "内存使用率 (%)", width: 120,
      cell: (item: ECWasteItem) => item.free_storage_or_memory != null ? item.free_storage_or_memory.toFixed(2) : "-",
    },
    {
      id: "evictions", header: "Evictions", width: 100,
      cell: (item: ECWasteItem) => item.evictions ?? "-",
    },
    {
      id: "cache_hit_rate", header: "缓存命中率", width: 110,
      cell: (item: ECWasteItem) => cacheHitRate(item),
    },
    {
      id: "estimated_monthly_savings", header: "预估月度节省 ($)", width: 130,
      cell: (item: ECWasteItem) => item.estimated_monthly_savings != null ? `${item.estimated_monthly_savings.toFixed(2)}` : "-",
    },
  ];
  ...
  return (
    <SpaceBetween size="l">
      ...
      } description="ElastiCache 闲置检测 — 识别低利用率的 ElastiCache 节点">
        ElastiCache 闲置资源
      </Header>

      <Container>
        <ColumnLayout columns={3} variant="text-grid">
          <div><Box variant="awsui-key-label">ElastiCache 闲置资源数</Box><Box variant="awsui-value-large">{filtered.length}</Box></div>
      ...
      {loading ? (
        ...
        <Table items={paged} columnDefinitions={columns} variant="full-page" stickyHeader
          ...
          empty={<Box textAlign="center" padding="xxl">暂无 ElastiCache 闲置资源</Box>}
          header={<Header counter={`(${filtered.length})`}>ElastiCache 闲置资源列表</Header>}
          pagination={
            <SpaceBetween direction="horizontal" size="xs">
              <Select
      ...
      <Modal visible={showWhitelistModal} onDismiss={() => setShowWhitelistModal(false)}
        ...
        <SpaceBetween size="m">
          <Box>即将把以下 <strong>{selectedItems.length}</strong> 个 ElastiCache 实例加入白名单：</Box>
          <Box>
