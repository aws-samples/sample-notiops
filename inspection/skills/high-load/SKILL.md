---
name: inspection-high-load
description: "Interpret pre-computed high-load and chronic-high inspection findings for RDS, Aurora, and ElastiCache (Redis, Valkey, Memcached). Use when a task description contains the marker NOTIOPS_INSPECTION together with hit_reason threshold_high or chronic_high. The deterministic rules already decided which resources breached which thresholds and at what severity; your job is to explain why — real degradation versus expected behaviour, correlated evidence, change attribution, blast radius at the limit, and one remediation action with its downtime cost. Read the resource's role from attrs.resource_role, never from lag metrics or names. Produces one compact section per finding_id."
---

# NotiOps Inspection — High Load & Chronic

You interpret findings that a deterministic rules engine has **already produced**. You are the
*why*, not the *what*. Read-only: describe and recommend, never modify a resource.

Scope: `hit_reason` containing `threshold_high` or `chronic_high`. Idle / cost-optimisation
findings go to the `inspection-cost-idle` skill instead.
...
1. **Orient.** Read `hit_reason`. `chronic_high` means the level has sat past the threshold
   for `consecutive_high_days` — **sitting at a bad level is more dangerous than drifting
   toward one**, because it is already at the pressure equilibrium and any spike crosses the
   line. A `chronic_high` finding may not have crossed the threshold today at all; do not
   dismiss it as healthy because the latest value looks acceptable.

2. **Fix the role before interpreting anything replication-shaped.** `attrs.resource_role` is
   the role; the shared "Evidence trust order" section is binding. Lag on a **known replica**
   is a health measure (near-zero = caught up, not unused); lag on a node whose role you have
   not established is not identity evidence, and Redis primaries publish `ReplicationLag`
   too. `metric_contract` marks these metrics `role_evidence: false` and tells you which
   metrics structurally cannot exist here (`applicable: false` + reason) — cite those as
   facts, not as gaps.

3. **Read `correlated` before forming any hypothesis.** This is where you add value the rules
   engine cannot:

   - `FreeableMemory` falling while `DatabaseConnections` stays flat → suspected leak or
     growing working set, not organic growth.
   - `FreeableMemory` falling *with* connections rising proportionally → organic growth;
     the fix is capacity, not a bug.
   - `FreeableMemory` low **and** `SwapUsage` rising → already swapping. Performance is
     *already* degraded; this is present tense, not a risk.
   - ElastiCache `Evictions` rising while hit rate falls → working set exceeded memory.
   - Redis/Valkey CPU: the judgement always uses `EngineCPUUtilization`, at every node size.
     ```
     Redis/Valkey  EngineCPUUtilization is the judged metric. The engine runs commands on a
                   single thread, so a saturated engine core hides behind a calm host average.
     Memcached     multi-threaded, no engine-thread metric — CPUUtilization is judged instead.
     ```
     `CPUUtilization` arrives in `correlated` for Redis/Valkey as **supporting evidence only**;
     it never produces a finding. Read the gap between the two rather than the host figure
     alone. `CPUUtilization` aggregates every core, including the dedicated threads that
     handle network I/O and TLS (enhanced I/O, on by default for Redis OSS 7.0.4+ and
     Valkey 7.2.6+), plus background work such as snapshots and `UNLINK`. On an idle
     2 vCPU node it was measured at roughly 4x the engine figure, so it is not a reliable
     indicator of engine capacity.
     ```
     engine high  + host high   engine-bound. Commands are the load — check SLOWLOG,
                                command complexity, data model.
     engine low   + host high   the load is NOT the command path. Suspect I/O threads,
                                TLS, or a snapshot in progress (SaveInProgress).
                                Do not recommend scaling for command throughput.
     engine high  + host low    normal on a large node: one saturated core out of many.
                                This is the case a host-average alarm would miss entirely.
     ```
     A burstable class (`cache.t3` / `cache.t4g`, visible in `attrs.instance_class`) throttles
     once CPU credits run out. `CPUCreditBalance` is in `correlated`; read its slope across
     `daily[]` for the burn rate. A balance trending to zero alongside high CPU means the
     ceiling is credit exhaustion, not vCPU count, and the fix is a non-burstable class rather
     than a bigger burstable one. On a non-burstable class the metric arrives as
     `not_applicable` — that is a fact, not a gap.
   - On a **known replica or reader** (step 2), lag moving with writer load → the replica
     cannot keep up; reads may be stale.
   - IOPS near the **real** baseline in `attrs`, not a gp3 assumption — a 100 GiB gp2 volume
     baselines at 300 IOPS, so 290 is saturated, not quiet.

   Stay inside the service you were given. On Memcached there is no `EngineCPUUtilization`,
   no `ReplicationLag`, no `DatabaseMemoryUsagePercentage`, no Performance Insights and no
   Redis single-thread story — pressure lives in host CPU (multi-threaded), memory and
   evictions (`BytesUsedForCacheItems`), connections and `CmdGet` / `CmdSet` traffic.
   Do not import another engine's checks or vocabulary into the section.

4. **Attribute change.** Check `change_events` — those are already collected for
   you and always available. Confirming with `rds:DescribeEvents` /
   `elasticache:DescribeEvents` (14 days, authoritative) or CloudTrail requires
   `resource_reachable: true`; when it is `false` those calls will fail or hit the
   wrong account, so rely on `change_events` and say that you could not
   cross-check.
   "FreeableMemory has fallen every day since the parameter-group change on the 12th" is a far
   better finding than the level alone. If no related event exists, write
   `change_attribution: no related change found` — do not invent a change because the metric
   moved, and do not attribute the shape to deployments remembered from Agent Space documents.

5. **Rule out warm-up.** If a restart, failover or instance-class change happened in the last
   3–5 days, a memory metric declining in a near-perfect straight line is most likely buffer
   pool and page cache refilling, **not** a leak. Check whether the last two days are
   flattening. Say `warm_up` and "re-check after <date>" rather than raising an alarm — Monday
   queues full of weekend-maintenance false alarms are how a whole report gets dismissed.

6. **State the blast radius concretely.** What actually happens at the limit: OOM restart?
   swap-driven latency? write failures (`maxmemory-policy: noeviction`)? connection refusal at
   `max_connections`? stale reads from a lagging replica? CPU-credit throttling? storage
   autoscaling that costs money rather than causing an outage (`attrs.max_allocated_storage`
   set)? Name the one mechanism the evidence supports — "performance risk" names none.

7. **Give one action with its downtime cost.** Instance-class changes and most parameter
   changes need a restart; `multi_az` softens but does not remove the impact. An action whose
   cost is unstated will not be executed, and a list of five alternatives is a menu, not a
   recommendation — pick the one the evidence supports. `approval_or_window` is `yes` for any
   class, parameter, topology, storage or failover change. Never call a restart or failover
   "no impact".
...
## <finding_id>
verdict: real_degradation | expected_behaviour | warm_up | insufficient_evidence
summary: <one or two sentences: what is breaching, and the most defensible cause>
evidence:
- <the breach: metric, stat, value vs threshold — numbers copied from the payload>
- <the decisive correlated fact>
- <role / change / capacity fact, if it changes the reading>
root_cause_hypothesis: <one supported cause, or "unknown">
blast_radius: <one concrete mechanism at the limit>
recommended_action:
- action: <one action>
...
Worked example — a burstable Redis node whose engine CPU is saturated:

```
## 444455556666#ap-northeast-1#elasticache#prod-cache-001#threshold_high#EngineCPUUtilization
verdict: real_degradation
summary: 引擎 CPU 已达 92%（阈值 80%），且 CPUCreditBalance 六天内从 288 降到 12，
  瓶颈是突发额度耗尽而不是 vCPU 数量。
evidence:
- EngineCPUUtilization Maximum 92.4% vs 阈值 80%，连续 6 天越线
- CPUUtilization 仅 31%：单线程引擎打满而整机均值看着平静，主机均值告警会漏掉
- CPUCreditBalance 日最小值 288 → 12，斜率持续为负，已接近节流
root_cause_hypothesis: 命令路径饱和叠加突发额度耗尽
blast_radius: credit 归零后 CPU 被压到基线，命令延迟整体抬升直至客户端超时
recommended_action:
- action: 换到非突发规格（cache.m7g.large 一档），换前用 daily[] 峰值核对目标容量
  downtime: yes
  approval_or_window: yes
  caveats: 换规格需重启，multi_az 只能减轻不能消除影响
change_attribution: no related change found
```

Note what the example does **not** contain: no configuration recap, no tour of every
correlated metric, no list of alternative remediation plans, and no closing summary
paragraph. Those are the shared Output discipline rules; this is what obeying them looks like.

One root-cause hypothesis; one blast-radius line. When a task carries **multiple** findings,
group the sections under two headings, in this order:

- **Present-state** — already past the limit, or headroom critical. Highest confidence.
- **Chronic** — `chronic_high`: has held a risky level for N days.

When the task carries a **single** finding, emit the one section with no group heading.
There is no predictive group. Do not create one.

## Mandatory counterexamples

These are real failure modes. Each one is binding.

**Redis primary publishing ReplicationLag.** `resource_role: redis_primary`, single-member
replication group, `ReplicationLag = [0,0,0]`. The role stays primary; the lag series must
not reclassify it as a replica, and there is no redundant read/HA path to cite.

**Low FreeableMemory with low disk reads.** Low `FreeableMemory` with low `ReadIOPS` is the
buffer pool doing its job (the companion evidence in `judgment` exists precisely for this).
Do not recommend a scale-up from the percentage alone.

**Redis host CPU high, engine CPU low.** Not command saturation. Look at I/O threads, TLS,
`SaveInProgress`, background work — and say which one the evidence supports. Recommending a
bigger node for command throughput here spends money without moving the bottleneck.

**Memcached finding.** No `EngineCPUUtilization`, no `ReplicationLag`, no Performance
Insights, no single-thread narrative. A Memcached section that mentions any of these was
written for the wrong engine.
...
- "according to the skill", a skill name, a step or line number, or an internal path such as
  `/aidevops/memory/...`;
- a tour of every correlated metric whether decisive or not;
- multiple alternative remediation plans for one finding;
- a time-to-failure estimate or exact action duration the payload does not contain;
- a conclusion about a sibling resource absent from the payload;
- "no impact" for a restart, failover, class change or parameter change.
